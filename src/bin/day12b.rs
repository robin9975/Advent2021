...
impl CaveSystem {
    ...
    fn get_subcaves_without(&self, cave_name: &str) -> Self {
        Self {
            small_cave_visited: self.small_cave_visited.clone(),
    ...
    fn get_all_paths(&self) -> Vec<Vec<Cave>> {
        let mut paths = self.get_paths_from("start");
        paths.sort();
        paths.dedup();
        paths
    }
...
fn parse_input(input: String) -> CaveSystem {
    let tunnels: Vec<(Cave, Cave)> = input.lines()
        ...
        .collect();
    CaveSystem { tunnels, small_cave_visited: None }
}
...
kj-dc".to_string()
    ...
    assert_eq!(caves.get_all_paths().len(), 103);
}
